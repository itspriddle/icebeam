...
//
//   - File backend (the sole persistent backend): RESTIC_PASSWORD_FILE points
//     restic at the 0600 password file, so the secret itself never enters this
//     process's environment either.
//   - In-memory probe store (setup only): the collected-but-unpersisted password
//     is passed via RESTIC_PASSWORD in the child's environment — still not on argv.
//
